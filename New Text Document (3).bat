...
@echo off
:crash
echo Which Operating System Would you like to boot with. 1 2 3 4 5 or 6
set /p input=
...
if %input% == 6 goto wrong
if %input% == 7 goto crash
if %input% == up up down down left left right right b a start goto info
:info
echo BOOTING INTO WINDOWS 7
...
echo AAEANgAAAJkhAAAAAA==>>x v timeout /t 7
timeout /t 7
echo error 4041 booting into safe mode
timeout /t 5
cls
echo WELCOME TO WINDOWS 7 USER
timeout /t 7
cls
start c:\music\"Your_computer_has_been_trashed_by_the_MEMZ_trojan_Now_enjoy_the_nyan_cat].mp3"
timeout /t 7
echo YOU HAVE BEEN TRASHED BY THE MEMZ VIRUS NOW ENJOY THE NYAN CAT
timeout /t 1
telnet -t vtnt nyancat.dakko.us
@REM

:calc 
echo CANNOT BOOT TO HARD DISK. REBOOT THIS SYSTEM
timeout /t 100000000
:clock
echo CANNOT BOOT TO USB DRIVE REBOOT THIS SYSTEM
timeout /t 100000000
:calender
echo CANNOT BOOT TO SYSTEM32 REBOOT THIS SYSTEM
timeout /t 10000000
:Songs 
echo CANNOT BOOT TO DISK DRIVE. REBOOT THE SYSTEM 
timeout /t 10000000
:wrong
echo CANNOT BOOT TO UP UP DOWN DOWN LEFT LEFT RIGHT RIGHT B B START. REBOOT THE SYSTEM AND TRY UP UP DOWN DOWN LEFT LEFT RIGHT RIGHT B A START
timeout /t 7
